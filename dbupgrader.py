...
def pyms_database_update():
    ...
    if dbversion == 1:
        ...
        database.commit()
        print('Database convertion to v2 complete')
    if dbversion == 2:
        sql_script = 'DROP VIEW IF EXISTS QNumbers;'
        database.execute(sql_script)
        sql_script = ('CREATE VIEW QNumbers AS SELECT identifier FROM batchsteps WHERE cycle = "Q-Standard" AND status '
                      '= 1 ORDER BY runnumber DESC LIMIT 1')
        database.execute(sql_script)
        cursor_obj.execute('UPDATE settings SET version = 3 WHERE version = 2')
        database.commit()
        print('Database convertion to v3 complete')
    database.close()
